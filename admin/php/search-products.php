...
function calculateRelevanceScore($productName, $categoryName, $description, $searchTerm) {
    ...
    if (mb_strlen($searchTermLower) === 1) {
        ...
        if (mb_strpos($categoryNameLower, $searchTermLower) !== false) {
            $score += 20;
        }
        return $score;
    }
    ...
    // Điểm cho danh mục phù hợp
    if (mb_strpos($categoryNameLower, $searchTermLower) !== false) {
        $score += 40;
    }
    
    // Điểm cho mô tả phù hợp
    if (mb_strpos($descriptionLower, $searchTermLower) !== false) {
        $score += 20;
    }
...
try {
    ...
    if (!empty($searchTerm)) {
        ...
        if (mb_strlen($searchTerm) === 1) {
            ...
            $stmt->bind_param('ssii', $searchPattern, $searchPattern, $itemsPerPage, $offset);
        } else {
            ...
            $stmt = $myconn->prepare($baseQuery . " ORDER BY 
                CASE 
                    WHEN p.ProductName LIKE ? THEN 1
                    WHEN p.ProductName LIKE ? THEN 2
                    WHEN c.CategoryName LIKE ? THEN 3
                    ELSE 4
                END,
                p.ProductName ASC 
                LIMIT ? OFFSET ?");
            $stmt->bind_param('sssssssis', 
                ...
                $searchPattern, $searchPatternNoAccents, $searchPattern,
                $itemsPerPage, $offset
            );
        }
    } else {
    ...
    if (!empty($searchTerm)) {
        if (mb_strlen($searchTerm) === 1) {
            $countStmt->bind_param('ss', $searchPattern, $searchPattern);
        } else {
